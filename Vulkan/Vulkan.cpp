...
private:
    ...
    void createTexture() {
        ...
        normalMap.image = Image(device, texWidth, texHeight, mipLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        transitionImageLayout(normalMap.image.Get(), VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
        copyBufferToImage(stagingBuffer.Get(), normalMap.image.Get(), static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        ...
        generateMipmaps(normalMap.image.Get(), VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels);

        normalMap.imageView = ImageView(device, normalMap.image.Get(), VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
        normalMap.sampler = Sampler(device, mipLevels);
    };
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            normalMapInfo.imageView = normalMap.imageView.Get();
            normalMapInfo.sampler = normalMap.sampler.Get();
    ...
    void cleanup() {
        ...
        vkDestroySampler(device.Get(), normalMap.sampler.Get(), nullptr);
        vkDestroyImageView(device.Get(), normalMap.imageView.Get(), nullptr);
        vkDestroyImage(device.Get(), normalMap.image.Get(), nullptr);
        vkFreeMemory(device.Get(), normalMap.image.GetMemory(), nullptr);
